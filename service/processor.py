...
class Processor:
	...
	def __init__(self):
		...
		file_path = path.join(script_dir, './resources/stopwords.txt')
	...
	def process(self, file, skip_words, core_words):
		analysis = analyze.parse_file(file, set()) if not skip_words else analyze.parse_file(file, self.words_to_skip) # list of tuples
